...
public interface BadgeRepository extends JpaRepository<BadgeModel,Long> {
    ...
    @Query(value = "SELECT b.* FROM project as p JOIN container as c on c.pk=p.container_pk JOIN badge as b on b.container_pk=c.pk where p.code=:code",nativeQuery = true)
